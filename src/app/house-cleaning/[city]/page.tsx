...
export default async function HouseCleaningCityPage({ params }: Props) {
  ...
  return (
    <main className="min-h-screen">
      <Navbar />
      <section className="bg-gradient-to-br from-deep-blue to-[#0a1e3f] py-16 lg:py-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="grid lg:grid-cols-2 gap-10 items-center">
            <div className="text-center lg:text-left">
              <div className="inline-flex items-center gap-2 bg-white/10 rounded-full px-4 py-2 mb-6">
                <span className="w-2 h-2 bg-soft-green rounded-full animate-pulse" />
                <span className="text-white/90 text-sm">Serving {cityName} & Surrounding Areas</span>
              </div>
              <h1 className="text-4xl sm:text-5xl font-extrabold text-white mb-4">House Cleaning in {cityName}</h1>
      ...
      <section className="py-16 bg-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6">
          <h2 className="text-3xl font-extrabold text-deep-blue mb-8 text-center">What&apos;s Included</h2>
          <div className="grid sm:grid-cols-2 gap-4">
            {["Kitchen & Bathrooms Deep Clean", "Dusting & Vacuuming All Rooms", "Floor Mopping & Sweeping", "Trash Removal", "Surface Sanitizing", "Mirror & Glass Cleaning"].map((f, i) => (
              <div key={i} className="flex items-center gap-3 bg-gray-50 rounded-xl p-4">
                <span className="text-soft-green font-bold text-lg">✓</span>
      ...
      <section className="py-16 bg-gray-50">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 text-center">
          <h2 className="text-3xl font-extrabold text-deep-blue mb-4">Trusted by {cityName} Residents</h2>
          <p className="text-gray-600 mb-8">See what your neighbors are saying.</p>
          <div className="grid sm:grid-cols-3 gap-4">
            {[
              ...
              <div key={i} className="bg-white rounded-xl p-5 shadow-sm">
                ...
                <p className="font-semibold text-deep-blue text-sm">{r.name}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-16 bg-deep-blue">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 text-center">
          <h2 className="text-3xl font-extrabold text-white mb-4">Get Your Free House Cleaning Quote in {cityName}</h2>
          <p className="text-white/80 mb-8">Response in under 15 minutes. No obligation.</p>
          <QuoteForm variant="compact" />
        </div>
      </section>
      <Footer />
